...
fn update_team_scores(mut scores: &mut HashMap<String, Team>,
                      team_name: String,
                      goals_scored: u8,
                      goals_conceded: u8)
{
    ...
    match scores.entry(team_name) {
        Vacant(hole) => {
            let new_team_info = Team {
                goals_scored,
                goals_conceded
            };

            hole.insert(new_team_info);
        },
        Occupied(mut slot) => {
            let mut team_info = slot.get_mut();
...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        // Update each team, reversing goals scored and goals conceded
